...
const parseGDBTable = (doc: Document): GDB[] => {
    const gdb: GDB[] = []
    ...
    for (const panel of panels) {
        ...
        const startDate =
            getText('span[id*="Label4_"]');

        const totalMarks =
            getText('span[id*="Label9_"]');

        const status =
            getText('span[id*="lblSubmissionStatus_"] > span:first-child') || "Not Submitted";

        const score =
            getText('span[id*="lblMarksObtained_"]');

        const isOpen =
            getText('span[id*="lblStatus_"]').toLowerCase() === "open";
