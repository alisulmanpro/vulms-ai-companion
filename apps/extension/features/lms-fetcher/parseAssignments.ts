...
const SELECTORS = {
  ...
  assignmentButtons:
    'input[type="image"][id^="MainContent_gvCourseList_ibtnAssignments_"]',
} as const;
...
const extractViewStateTokens = (doc: Document) => ({
  viewState:
    doc.querySelector<HTMLInputElement>(SELECTORS.viewState)?.value ?? "",
  vsGenerator:
    doc.querySelector<HTMLInputElement>(SELECTORS.vsGenerator)?.value ?? "",
});

const extractCourseList = (doc: Document): CourseInfo[] => {
  const courses: CourseInfo[] = [];
  const buttons = doc.querySelectorAll<HTMLInputElement>(
    SELECTORS.assignmentButtons,
  );
  ...
  for (const btn of buttons) {
    ...
    const trackingSpan = doc.querySelector<HTMLSpanElement>(
      `#MainContent_gvCourseList_lblTracking_${index}`,
    );
    const titleText =
      (
        trackingSpan?.closest("h3")?.firstChild as Text | null
      )?.textContent?.trim() ?? "";
...
const fetchHomePage = async (): Promise<HomeFetchResult> => {
  ...
  const hfCourseCode =
    doc.querySelector<HTMLInputElement>(SELECTORS.hfCourseCode)?.value ?? "";

  if (!viewState)
    throw new Error(
      "[Assignments Scrapper] __VIEWSTATE not found — session expired",
    );

  const courses = extractCourseList(doc);
  if (courses.length === 0)
    throw new Error("[Assignments Scrapper] No courses found");

  return { viewState, vsGenerator, hfCourseCode, courses };
};

const parseAssignmentTable = (
  doc: Document,
  course: CourseInfo,
): Assignment[] => {
  ...
  for (const panel of panels) {
    ...
    const title = getText('span[id*="Label3_"]');

    const dueDate = getText('span[id*="lblDueDate_"]');

    const totalMarks = getText('span[id*="lblTotalMarks_"]');

    const status = getText('span[id*="lblsubmitted_"]') || "Not Submitted";

    const score = getText('span[id*="lblScore_"]');

    const downloadUrl =
      panel
        .querySelector<HTMLAnchorElement>('a[id*="lbtnViewSubmittedFile_"]')
        ?.getAttribute("href") ?? "";

    const commentsUrl =
      panel
        .querySelector<HTMLAnchorElement>('a[href*="InstructorComments.aspx"]')
        ?.getAttribute("href") ?? "";
...
const fetchCourseAssignments = async (
  ...
  return {
    assignments: parseAssignmentTable(listDoc, course),
...
const parseAssignments = async (): Promise<CourseAssignments> => {
  ...
  for (const course of initial.courses) {
    console.log(
      `[Assignments Scrapper] < ${course.index + 1}/${initial.courses.length} > Fetching ${course.courseCode}...`,
    );

    try {
      const { assignments, newViewState, newVsGenerator } =
      ...
      console.error(
        `[Assignments Scrapper] Error processing ${course.courseCode}:`,
        error,
      );
