...
const parseQuizTable = (doc: Document, course: CourseInfo): Quiz[] => {
  const quizzes: Quiz[] = [];
  ...
  for (const panel of panels) {
    ...
    const title = getText('span[id*="lblTitle_"]');

    const dueDate = getText('span[id*="lblEndDate_"]');

    const startDate = getText('span[id*="lblStartDate_"]');

    const totalMarks = getText('span[id*="lblTotalMarks_"]');

    const status =
      getText('span[id*="lblSubmitted_"] > span:first-child') ||
      "Not Submitted";

    const score = getText('span[id*="lblGetMarks_"]');

    const isOpen = getText('span[id*="lblStatus_" i]').toLowerCase() === "open";
...
const fetchCourseQuiz = async (
  ...
  return {
    quizzes: parseQuizTable(listDoc, course),
...
const parseQuiz = async (): Promise<CourseQuiz> => {
  ...
  for (const course of initial.courses) {
    console.log(
      `[Quiz Scrapper] < ${course.index + 1}/${initial.courses.length} > Fetching ${course.courseCode}...`,
    );
    try {
