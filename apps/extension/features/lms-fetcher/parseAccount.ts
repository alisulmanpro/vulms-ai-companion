...
const parseAccount = async (): Promise<AccountSummary | null> => {
  try {
    ...
    if (!response.ok) {
      console.error(
        `[Account Scraper] Request failed with status ${response.status}`,
      );
      return null;
    }
    ...
    const panels = doc.querySelectorAll(
      '[id^="MainContent_grdaccountbook_pnl_"]',
    );
    ...
    panels.forEach((pnl) => {
      ...
      // Dates Selectors
      const paidDateEl = pnl.querySelector<HTMLElement>(
        `#MainContent_grdaccountbook_lblpaiddate_${idSuffix}`,
      );
      ...
      // Challan Number
      const challanNoEl = pnl.querySelector<HTMLElement>(
        `#MainContent_grdaccountbook_lbl_${idSuffix}`,
      );
      if (!challanNoEl) return;
      const challanNo = challanNoEl.textContent.trim();

      // Original Payable Fee
      const payableAmountEl = pnl.querySelector<HTMLElement>(
        `#MainContent_grdaccountbook_lblPayableAmount_${idSuffix}`,
      );
      let payableFee =
        parseInt(payableAmountEl?.textContent?.trim() ?? "0", 10) || 0;

      const dueDateEl = pnl.querySelector<HTMLElement>(
        `#MainContent_grdaccountbook_lblduedate_${idSuffix}`,
      );
      ...
      const printBtn = pnl.querySelector<HTMLInputElement>(
        `#MainContent_grdaccountbook_hlPrint_${idSuffix}`,
      );

      if (printBtn) {
        const onClickAttr = printBtn.getAttribute("onclick") || "";
        const urlMatch = onClickAttr.match(
          /["'](ChallanPrintPreview\.aspx\?[^"']+)["']/,
        );
    ...
    const grandTotalFees = challans.reduce(
      (sum, item) => sum + item.payableFee,
      0,
    );
